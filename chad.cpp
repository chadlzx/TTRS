...
struct ticket{
    ...
    String<12> data_start,data_arrive,data;
    ...
    void print2(){
     ...
     void input2(train& now,int s,int t,String<12> Data){
		...
		for(int i=1;i<=s;i++){
			if(now.station[i].arrive_time>now.station[i].start_time||now.station[i].arrive_time<now.station[i-1].start_time)
				Data.improve();
		}
		data_start=Data;
        for(int i=s+1;i<=t;i++){
...

{
    ...
    t.num_buy[k]+=num;
    for(int i=1;i<=k1;i++){
			if(now.station[i].arrive_time>now.station[i].start_time||
				now.station[i].arrive_time<now.station[i-1].start_time)
				data.improve();
	}
    ...
    for(int i=k1+1;i<=k2;i++){
    	if(now.station[i].arrive_time>now.station[i].start_time||
				now.station[i].arrive_time<now.station[i-1].start_time)
				data.improve();
        if(now.station[i].num_remain[data.trans2()][k]>=num)
            now.station[i].num_remain[data.trans2()][k]-=num;
        else return 0;
    }
